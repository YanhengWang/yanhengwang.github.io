...
For our purpose, we want to drop the restriction of "passing through the origin" while maintaining flatness:

**Definition.**
An *affine space* is a shifted linear space, that is $$V + \vec{u} := \set{ \vec{v}+\vec{u} : \vec{v} \in V}$$ for some linear space $$V$$ and vector $$\vec{u}$$.

Given an affine space $$U$$, we may "undo" the shift and recover the (unique) underlying linear space by taking $$U - \vec{u}$$ for an *arbitrary* vector $$\vec{u} \in U$$. The argument is one-liner algebra, which we leave as an exercise.

In the sight of this one-one correspondence, concepts for linear spaces easily and naturally transfer to the context of affine spaces.

- The *dimension* of an affine space is defined as the dimension of its underlying linear space.
- The *affine span* (or *affine hull* in standard terms) of $$S \subseteq \Real^d$$ is defined as $$\aff(S) := \vec{s} + \span (S - \vec{s})$$, where the choice of $$\vec{s} \in S$$ is immaterial. (Prove it!) Essentially, we pretend $$\vec{s}$$ as the origin, extend from there a flat space, and then restore the true origin.
...
A *$$k$$-simplex* is the convex hull of $$k+1$$ affinely independent points. It is a V-polytope of dimension $$k$$ (why?). Since every V-polytope of dimension $$k$$ has at least $$k+1$$ defining points, a $$k$$-simplex is indeed the simplest one among all candidates, hence its name.
