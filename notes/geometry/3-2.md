...
Here we invented a new dimension and lifted the defining points of $$P$$ (hence also the entire polytope) at height 1; see the illustration below. By geometric intuition we should be able to recover $$P$$ by slicing $$C$$ at height 1, that is
...
There is nothing special behind the scenes. $$D$$ was defined in a way that the value of $$\vec{x}$$ determines a particular threshold above which the $$\vec{y}$$ can freely move. We specified our conic span to capture the threshold (via the first $$2d$$ vectors), and to give freedom of moving above it (via the remaining vectors).
...
This in implies $$\sum_{i=1}^{\ell} \lambda_i = \sum_{j=\ell+1}^r \lambda_j =: \Lambda$$, and
