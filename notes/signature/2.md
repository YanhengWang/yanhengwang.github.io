...
- Here as always, the protocol executes in line order, and exactly one party is activated at any instant. The ► symbol declares that some external variable becomes accessible to the current party from that point on. For example, the "select" step cannot access external variable; the "forge" step, on the other hand, has access to $$J.pk$$ and $$J.\sigma_1, \dots, J.\sigma_\ell$$. All access are read-only.
