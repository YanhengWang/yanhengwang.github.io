...
The aim of this series is to argue the contrary -- rigorously based on a couple of yet unproven assumptions that people strongly believe. The reader should not be discouraged by such "unreliable" axioms. In fact, traditional signatures never enjoyed a systematic theory; what we try to establish here already far transcends the tradition. It is at least an attempt of formalisation, of understanding what's going on. Moreover, many notions that pop up during our development are of independent interest.
...
- The adversary $$A$$ in the protocol is generic as the "select" and "forge" methods are not yet specified. A concrete adversary can implement them at his disposal. But we stress that he is *not* allowed to alter the protocol, e.g. access barred information, change the timeline or modify the behaviour of judge.
...
Clearly $$\Sigma'$$ is sound. But it is *not* $$\exists\CM^+$$ secure: After the adversary learns $$pk' = pk \Vert a \Vert b$$, he may enquire about the signature of $$a$$ and, as a byproduct, learn $$\eta = \sgn(sk,b)$$. Since  $$b$$ is an unseen message, the pair $$(b,\eta \Vert \mathbf{0})$$ constitutes a valid forgery.
