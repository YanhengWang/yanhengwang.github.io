...
Therefore, even if we had the much more moderate goal of constructing a $$?\textsf{NM}$$ secure scheme, we *must* assume some highly non-trivial and unproven complexity result. We defer its proof to the next time when we introduce the concept of one-way functions.
