...
Without the knowledge of $$\beta$$, how can we respond the signature of $$\widetilde{m}$$? The trick is to generate the answer $$\sigma$$ straight away, which determines a unique $$a := g^m \cdot b^{\widetilde{\sigma}}$$ backwards. This in turn determines a unique $$\alpha: g^\alpha = a$$ backwards. Due to bijectivity, such sampling is *equivalent to* the standard forward procedure $$\alpha \in \Int_n \to a \to \sigma$$, and the adversary will not perceive a difference.
